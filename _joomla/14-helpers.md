...
You can think of these Jekyll helpers as little shortcuts. Since GitHub Pages doesn't allow most plugins --- [custom tags](https://jekyllrb.com/joomla/plugins/#tags) are out. Instead the theme leverages [**includes**](https://jekyllrb.com/docs/templates/#includes) to do something similar.
...
**More Gallery Goodness:** A few more examples and [source code](https://github.com/{{ site.repository }}/blob/master/joomla/\_posts/2010-09-09-post-gallery.md) can be seen in [this sample gallery post]({{ "" | relative_url }}{% post_url 2010-09-09-post-gallery %}).
{: .notice--info}

## Feature row

Designed to compliment the [`splash`]({{ "/joomla/layouts/#splash-page-layout" | relative_url }}) page layout as a way of arranging and aligning "feature blocks" containing text or image.
...
| **btn_class**     | Optional     | Button style. See [utility classes]({{ "/joomla/utility-classes/#buttons"                              | relative_url }}) for options.      | `btn` |
...
**More Feature Row Goodness:** A [few more examples]({{ "/splash-page/" | relative_url }}) and [source code](https://github.com/{{ site.repository }}/blob/master/joomla/\_pages/splash-page.md) can be seen in the demo site.
{: .notice--info}
...
**1.** Start by adding a set of titles and URLs to `_data/navigation.yml` in the same way the [`main` navigation]({{ "/joomla/navigation/" | relative_url }}) is built.
...
**ProTip:** If you're applying the same navigation list to several pages setting it as a [Front Matter default](https://jekyllrb.com/joomla/configuration/#front-matter-defaults) is the better option.
{: .notice--info}

The theme's documentation is built with the `nav_list` helper so if you'd like an example to dissect take a look at `navigation.yml`, `_config.yml` and `_doc` collection in the [`/joomla/` folder](https://github.com/{{ site.repository }}/tree/master/docs/) of this repo.
